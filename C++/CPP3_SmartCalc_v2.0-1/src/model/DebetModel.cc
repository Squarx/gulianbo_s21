...
void DebetModel::ProcessPut(
  ...
  if (!list_put.empty() && freq_put_ != kNo &&
    ...
    if (curr_put_date->first != start_date_) {
      ...
      renderer_.push_back({curr_put_date->first.toString("dd-MM-yyyy"), "", "",
                           QString::number(curr_put_date->second),
                           QString::number(deposit_)});
    }
    ++curr_put_date;
  }
}
...
void DebetModel::ProcessPayment(const QDate& current_date,
  ...
  if (current_date == *curr_payment_date) {
    ...
    renderer_.push_back({current_date.toString("dd-MM-yyyy"),
                         QString::number(daysDifference_periodic),
                         QString::number(periodic_percents),
                         QString::number(periodic_percents),
...
void DebetModel::Calculate() {
  ...
  for (auto curr_date = start_date_; curr_date <= end_date_;) {
    ...
    //        all_persents += std::round(percents * 100.0) / 100.0;
    //        periodic_percents += std::round(percents * 100.0) / 100.0;
    all_persents += percents;
    periodic_percents += percents;
    // Process Пополнение счета
    ProcessPut(curr_date, curr_put_date);
    // Process Вычет co счета
    ProcessWithdraw(curr_date, curr_withdraw_date);
    // Process Выплаты по счету
...
void DebetModel::GetFinalResult() {
  ...
  output[0] = QString::number(all_persents, 'f', 2);
  if (all_taxes != 0) {
    output[1] = QString::number(all_taxes, 'f', 2);

  } else {
    output[1] = QString("Не облагается налогом (меньше (%1))")
                    .arg((tax_rate_ / 100.0) * 1e6);
  }
  output[2] = QString::number(deposit_, 'f', 2);
}
