...
S21Matrix::S21Matrix(const double *data, int rows, int cols)
  ...
  for (auto i = 0; i < rows; ++i) {
    for (auto j = 0; j < cols; ++j) {
      this->_matrix[i][j] = data[i * cols + j];
    }
  }
}
...
bool S21Matrix::EqMatrix(const S21Matrix &other) {
  ...
  for (int i = 0; res && i < _rows; i++)
    for (int j = 0; res && j < _cols; j++)
      if ((fabs(_matrix[i][j] - other._matrix[i][j])) >= 1e-7) res = false;
  return res;
}
...
void S21Matrix::SumMatrix(const S21Matrix &other) {
  ...
  for (auto i = 0; i < _rows; i++)
    for (auto j = 0; j < _cols; j++) _matrix[i][j] += other._matrix[i][j];
}
...
void S21Matrix::MulNumber(const double num) {
  for (auto i = 0; i < _rows; i++)
    for (auto j = 0; j < _cols; j++) _matrix[i][j] *= num;
}
...
S21Matrix S21Matrix::CalcComplements() {
  ...
  if (_rows == 1) {
    COMP._matrix[0][0] = this->_matrix[0][0];
  } else {
  ...
  for (int i = 0, minor_row = 0; i < _rows; i++) {
    if (i != row_skip) {
      for (int j = 0, minor_col = 0; j < _cols; j++)
        if (j != col_skip)
          minor._matrix[minor_row][minor_col++] = _matrix[i][j];
      minor_row++;
    }
  }
  return minor;
...
double S21Matrix::Determinant() {
  double res = 0;
  if (_rows != _cols) throw(std::out_of_range("NOT SQUARE"));
