  ...
  if (!list_put.empty() && freq_put_ != kNo &&
    ...
    if (curr_put_date->first != start_date_) {
      ...
      renderer_.emplace_back(
          std::array<QString, 5>{curr_put_date->first.toString("dd-MM-yyyy"),
                                 "", "", QString::number(curr_put_date->second),
  ...
  if (!list_withdraw.empty() && freq_withdraw_ != kNo &&
    ...
    if (curr_withdraw_date->first != start_date_) {
      ...
      renderer_.push_back({curr_withdraw_date->first.toString("dd-MM-yyyy"), "",
                           "", message, QString::number(deposit_, 'f', 2)});
    }
...
void DebetModel::ProcessPayment(const QDate& current_date,
  ...
  if (current_date == *curr_payment_date) {
    ...
    if (capitalize_) {
      deposit_ += periodic_percents;
    }

    renderer_.emplace_back(
        std::array<QString, 5>{current_date.toString("dd-MM-yyyy"),
                               QString::number(daysDifference_periodic, 'f', 2),
                               QString::number(periodic_percents, 'f', 2),
                               QString::number(periodic_percents, 'f', 2),
                               QString::number(deposit_, 'f', 2)});
...
void DebetModel::ChangeRate(double& rate, int& curr_year_capacity,
                            QDate curr_date, int& start_year) {
  if (curr_date.year() != start_year) {
    curr_year_capacity = curr_date.daysInYear();

    rate = (interest_rate_ / (static_cast<double>(curr_year_capacity) * 100.));

    start_year = curr_date.year();
  }
}

void DebetModel::CalculateDebet() {
  ...
  for (auto curr_date = start_date_; curr_date <= end_date_;) {
    ChangeRate(rate, curr_year_capacity, curr_date, start_year);
    ...
    ProcessPayment(curr_date, curr_key_date, periodic_date, periodic_percents);
...
void DebetModel::Calculate() noexcept {
  Cleaner();
...
void DebetModel::Cleaner() noexcept {
  list_payment.clear();
  list_put.clear();
  list_withdraw.clear();
  renderer_.clear();
