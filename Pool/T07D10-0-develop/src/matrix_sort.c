...
void sort_slow_static(int height, int weight, int a[height][weight]);
void copy(int a[100], int b[100], int n);
int** sort_slow(int** a, int height, int weight);
    ...
    switch (input) {
        case 1 : {
            ...
            if (!flag) {
                ...
                sort_slow_static(height, weight, arr_stat);
        ...
        case 2 : {
            ...
            if (!*flag) break;
            if (!*flag) break;
            sort_slow(my_arr, height, weight);
        ...
        case 4 : {
            ...
            Dy_writer(height, weight, my_arr, flag);
            if (!*flag) break;
            int** hh = sort_slow(my_arr, height, weight);
            Dy_printer(height, weight, hh);
            Dy_cleaner3(hh);
            break;
        }
        default : {
...
int** sort_slow(int** a, int height, int weight) {
    ...
    for (int i = 0; i < height ; i++) {
        for (int j = 0; j < height - 1 ; j++) {
            if (sum(a[j], weight) < sum(a[j+1], weight)) {
                copy(a[j], tmp, weight);
                copy(a[j+1], a[j], weight);
                copy(tmp, a[j+1], weight);
            }
        }
    }
    return a;
