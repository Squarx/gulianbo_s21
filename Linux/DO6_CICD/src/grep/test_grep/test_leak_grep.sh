...
testing()
{
    ...
    then
      (( SUCCESS++ ))
        echo "$IRed$FAIL$NC $IBlue$SUCCESS$NC $IPurple$COUNTER$NC ${On_Green}SUCCESS$NC grep $t"
    else
      (( FAIL++ ))
        echo "$IRed$FAIL$NC $IBlue$SUCCESS $IPurple$COUNTER$NC $On_Red FAIL $NC grep $t"
#        echo "$leak"
    fi
